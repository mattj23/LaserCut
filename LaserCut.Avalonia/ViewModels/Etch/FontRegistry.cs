...
public class FontRegistry : ReactiveObject
{
    ...
    public FontItem AddNew(FontFamily family, double size)
    {
        var id = NextId();
    ...
    public FontItem AddNew(string family, double size)
    {
        return AddNew(new FontFamily(family), size);
    }

    public bool Remove(FontItem item)
    {
    ...
    public FontItem AddNew()
    {
        if (_fonts.Count == 0)
        {
            return AddNew(SystemFonts.Instance.Default, 12);
        }

        return AddNew(_fonts.Last().Family, _fonts.Last().Size);
    }

    private FontItem AddItem(FontItem item)
    {
